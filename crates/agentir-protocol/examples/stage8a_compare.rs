...
const SEMANTIC_FILES: &[&str] = &[
    "cpu-saxpy.json",
    "cpu-scalar-elementwise.json",
    "cpu-rejected-reduction.json",
    "summary.json",
];
...
fn main() -> Result<(), String> {
    ...
    if args.len() != 2 {
        return Err("usage: stage8a_compare RUN_1 RUN_2".to_owned());
    }
    ...
    for name in SEMANTIC_FILES {
        ...
        {
            return Err(format!("Stage 8A semantic file differs: {name}"));
        }
    }
    println!(
        "{}",
        serde_json::to_string(&json!({
            "schema_version": "agentir.stage8a.compare.v1",
