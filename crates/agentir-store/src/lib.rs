...
use agentir_core::{
    ...
    persistence::{
        ...
        migrate_snapshot_v5, migrate_snapshot_v6,
    },
...
pub type WorkspaceArchive = WorkspaceArchiveV7;
...
fn current_archive(snapshot: WorkspaceSnapshot) -> AgentResult<WorkspaceArchiveV7> {
...
fn encode_archive(archive: &WorkspaceArchiveV7) -> AgentResult<Vec<u8>> {
...
pub fn migrate_archive_v1_to_v2(archive: WorkspaceArchiveV1) -> AgentResult<WorkspaceArchiveV2> {
    ...
    let (workspace, _replay) = Workspace::from_legacy_migrated_snapshot(migrated_v7)?;
    ...
    match decoded {
        ...
        DecodedArchive::V5(archive) => {
            ...
            let report = MigrationReport {
                ...
                target_archive_version: ARCHIVE_FORMAT_VERSION,
                applied_steps: vec![MIGRATION_V5_TO_V6.to_owned(), MIGRATION_V6_TO_V7.to_owned()],
                workspace: migrated.workspace.clone(),
                head: migrated.head.clone(),
                revisions: migrated.revisions.len(),
                old_archive_hash,
        ...
        DecodedArchive::V6(archive) => {
            ...
            let report = MigrationReport {
                ...
                target_archive_version: ARCHIVE_FORMAT_VERSION,
                applied_steps: vec![MIGRATION_V6_TO_V7.to_owned()],
                workspace: migrated.workspace.clone(),
                head: migrated.head.clone(),
                revisions: migrated.revisions.len(),
                old_archive_hash,
        ...
        DecodedArchive::V7(archive) => {
            ...
            let report = MigrationReport {
                ...
                target_archive_version: ARCHIVE_FORMAT_VERSION,
                applied_steps: vec![MIGRATION_V7_NOOP.to_owned()],
                workspace: archive.snapshot.workspace.clone(),
                head: archive.snapshot.head.clone(),
                revisions: archive.snapshot.revisions.len(),
                old_archive_hash: archive.archive_hash,
...
/// Encodes a workspace as current archive format version 7 without filesystem I/O.
pub fn encode_workspace_archive(workspace: &Workspace) -> AgentResult<Vec<u8>> {
    encode_archive(&current_archive(workspace.snapshot())?)
}

/// Writes a checksummed v7 workspace archive using a same-directory temporary file and rename.
pub fn save_workspace(
    ...
    Ok(metadata_v7(&archive, encoded.len()))
}
...
/// Fully verifies a source archive and atomically writes its current v7 representation.
