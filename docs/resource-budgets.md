...
Stage 7A separates algorithmic envelope from operational safety. Beam width, semantic depth, children retained, expansion order and checkpoint cadence are versioned `SearchPlan` semantics. Runtime caps on nodes, edges, semantic expansions, checkpoint bytes, isolated engines and production requests are safety policy and enter no objective, plan, node, trace or result hash. Search counts continuation frames/pages, choices, inferences/scores, isolated engines, production requests, accepted/compiler/policy-rejected expansions, repairs, nodes/edges, duplicates, comparisons, checkpoints, replayed nodes and archive bytes with checked arithmetic. Exact checkpoint byte limit accepts; limit plus one rejects before publication.
