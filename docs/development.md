...
Stage 7C offline readiness uses `cargo test -p agentir-policy-eval --test stage7c`, two `stage7c_study --output target/stage7c-study/run-{1,2}` runs, and `stage7c_compare`. The comparator requires 14 semantic/acquisition/cohort/recommendation/archive-v6 files to be byte-identical and excludes `timing-observations.json`. The optional real-device output stays under `target/stage7c-study/device/` and is never committed.

Stage 7D offline readiness uses `cargo test -p agentir-policy-eval --test stage7d`, two `stage7d_study --output target/stage7d-study/run-{1,2}` runs, and `stage7d_compare`. The comparator requires 13 recovery/acquisition/cohort/recommendation/archive-v7 semantic files to be byte-identical and excludes wall-clock observations. Optional device output stays under `target/stage7d-study/device/`; a missing production adapter/workspace is a structured skip.

Stage 7E offline readiness uses `cargo test -p agentir-policy-eval --test stage7e`, two `stage7e_study --output target/stage7e-study/run-{1,2}` runs, and `stage7e_compare`. The study requires at least two distinct production-replayed terminal artifacts before its labelled synthetic acquisition, retains explicit terminal/artifact/slot/recovery/replay/archive metrics, and compares nine semantic files byte-for-byte. Controlled device output stays under `target/stage7e-study/device/` and cannot be replaced by synthetic evidence.
...
## Stage 7E campaign checks

Run `cargo test -p agentir-policy-eval --test stage7e`, then run `stage7e_study` twice and compare with `stage7e_compare`. Semantic files must be byte-identical; `operational.json` is deliberately excluded. Real-device material belongs only under `target/stage7e-study/device/` and must never be committed.
# Stage 4 checks
