...
Stage 6B adds independent choice-ID, choice-set, feature-schema, ranking-policy, ranking-trace and selection v1 domains, plus ranked episode v2 and evaluation archive v2. None enters content/spec/impl/candidate/equality/memory/schedule/backend/artifact identities. Choice identity excludes compiler order and transport-only request/correlation/timing/host/provider-session metadata.
