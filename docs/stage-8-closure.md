...
- Synthetic measurement orchestration evidence uses explicit clock and execution doubles in an isolated workspace/store. It checks exact acquisition call counts, integer samples and aggregates, all independent record anchors, atomic rejection, structural query/check, corruption rejection, and archive v11 replay. Structural paths accept no clock or executor capability, and replay reuses archived revision timestamps.
