...
`agentir.evaluation.archive` version 1 is separate from workspace archive v10. It stores an `EvaluationManifest`, exact corpus, policy descriptors, compiler build hash, optional source workspace hashes, ordered runs/episodes/transcripts/outcomes, raw aggregates, optional hardware anchors, and `archive_hash`.
